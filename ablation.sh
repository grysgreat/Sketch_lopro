...
#       echo ">>>>>>>> model: $testmodle; FP16" 
#       echo ">>>>>>>> bit = $bit"

#       echo ">>>>>>>>Starting test ppl: $testmodle, lora_bit:$low_bit, fix_rank:$rank, method:$method"
#       CUDA_VISIBLE_DEVICES=0 python $ppl_eval_dir/llama.py "$save_dir/${testmodle}" "wikitext2"

#       echo ">>>>>>>>Starting zero shot task: $testmodle, lora_bit:$low_bit, fix_rank:$rank, method:$method"
#       HF_DATASETS_CACHE="/root/.cache/huggingface/datasets" lm_eval --model hf  --model_args "pretrained=$data_base_dir/${testmodle}"  --tasks arc_challenge,arc_easy,winogrande,openbookqa
#     done
#   done
# done
