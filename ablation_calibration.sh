...
declare -a id_bsizes=(256)
declare -a ha_bsizes=(256)
...
for bit in "${qbit_s[@]}"; do
  for testmodle in "${models[@]}"; do
    ...
    echo "$divider"
    for id_bsize in "${id_bsizes[@]}"; do
      for ha_bsize in "${ha_bsizes[@]}"; do
        echo ">>>>>>>> model: $testmodle; bit = $bit, method = $method, lora_bit=$low_bit, id_bsize=$id_bsize, ha_bsize=$ha_bsize" 
        echo ">>>>>>>> bit = $bit"
